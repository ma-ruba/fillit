...
char	**ft_make_map(int piece_count) // создается двумерная карта, где будут расставляться тетримины (норма)
{
	char **map;
	int	count;
	int	map_size;
...
int	ft_map_size(char **map) // рассчитывается размер карты (норма)
{
	int	map_size;
...
char	**ft_enlarge_map(int map_size) // расширение карты после того как не удалось разместить тетримины (норма)
{
	...
	count = -1;
	while (++count < map_size)
		map[count] = ft_memset(map[count], '.', map_size); // не уверена что правильно работает
	return (map);
}
